...
**Pagine**: musei (lista a card + dettaglio/editing, creazione riservata a `super_admin`), contenuti (opere con item annidati in righe espandibili, regola di business "un'opera si pubblica solo se ha almeno un item"), **Visit Builder** (catalogo item a sinistra raggruppato per opera, letto scorrendo tutte le pagine perché il backend taglia a 100 per pagina; una tappa = un'opera con la lista piatta delle sue varianti — `VisitStep.itemIds`: il click su un item crea la tappa dell'opera o vi aggiunge la variante; più item dello stesso registro convivono purché di durata diversa, e l'unico conflitto è la cella già occupata — stesso registro **e** stessa durata — risolta da un modal di preview in sola lettura (`components/itemPreview.js`, riusabile). Ogni tappa mostra la griglia registro × durata che il player navigherà: una riga per registro, un chip per durata, in grigio i registri con una sola durata; sequenza step a destra con riordino manuale a frecce su/giù; salvataggio atomico dell'intero array `steps` in un'unica `PUT`, nessun autosave), utenti (solo `super_admin`, sospensione via `PATCH {status}` invece di cancellazione — non esiste `DELETE /users`).
