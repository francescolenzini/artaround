...
**Mappa multi-piano con pin**: `Artwork.location` è la fonte unica `{ label, floor, x, y }` (percentuali sull'immagine); ogni tappa-opera conserva `artworkId` e `GET /visits/:id` aggiunge `mapLocation` solo alla risposta. Convenzione interna: `floor: 1` = sale 1-45 ("Secondo piano" Uffizi), `floor: 2` = sale 46-101 ("Primo piano" Uffizi). Le immagini di sfondo vivono in `services/navigator/app/public/maps/` (`uffizi-p1.png`, `uffizi-p2.png`). Il componente mostra un selettore di piano e pin posizionati con CSS assoluto; il click su un pin apre una card con titolo, sala e bottone per saltare a quello step nel player.

**Due assi di adattamento nel player** (dal 2026-07-26, prima era il solo registro): ogni step espone `itemIds`, la lista piatta delle varianti dell'opera disponibili in quella tappa; il player le carica in **una sola richiesta** (`GET /artwork-items?id=a,b,c`) e ne costruisce una griglia **registro × durata** leggendo `classification.languageRegister` e `classification.fruitionLength` da ciascuna. L'asse registro è la scala ordinale dell'enum; l'asse durata non ha scala predefinita — i gradini sono i minuti realmente caricati per quella tappa.

I comandi si dividono di conseguenza: `"non capisco"`/`"troppo semplice"` muovono il **registro**, `"dimmi di meno"`/`"dimmi di più"` la **durata**. I due assi non sono simmetrici: un salto di registro viene sempre onorato adattando la durata alla più vicina disponibile, mentre un salto di durata resta dentro lo stesso registro e fallisce esplicitamente se lì non c'è nulla (rispondere con un testo di un altro registro sarebbe la confusione fra i due assi che la separazione esiste per togliere). Entrambi aggiornano **insieme** schermo (`screenText`) e sintesi (`ttsText`).

Tutta la logica di selezione sta in `src/lib/itemVariants.ts` (`buildGrid`, `resolveVariant` con ripiego alla cella più vicina, `neighbour`), modulo puro coperto da `itemVariants.test.ts` (vitest). La preferenza `{registro, durata}` è persistita in `sessionStorage` e sopravvive ai reload. Chip e bottoni restano tappabili anche quando il comando non è disponibile (`aria-disabled`, non `disabled`), così il Toast esplicativo — che nomina l'asse saturo — resta raggiungibile anche senza usare la voce. Gli item scaricati sono cache-ati per id.
...
**Pagine**: musei (lista a card + dettaglio/editing, creazione riservata a `super_admin`), contenuti (opere con item annidati in righe espandibili, collocazione strutturata sala/piano/coordinate e regola di business "un'opera si pubblica solo se ha almeno un item"), **Visit Builder** (layout desktop master–detail: sequenza di step a sinistra e composer degli item sempre visibile a destra, con pannelli allineati in altezza. L'intera card seleziona/espande lo step; una tappa conserva `artworkId` e la lista piatta `itemIds`, distingue principale/opzionale con un toggle e presenta gli item ordinati per registro e durata, senza matrice. Il composer offre ricerca, filtri e selezione multipla; una nuova tappa nasce vuota e va composta esplicitamente. Gli step si riordinano con posizione numerica o drag-and-drop, mentre apertura e chiusura seed restano protette ai bordi. La coppia registro+durata è univoca nella tappa sia nella UI sia nella validazione API. Il salvataggio dell'intero array `steps` resta atomico in un'unica `PUT`, senza autosave), utenti (solo `super_admin`, sospensione via `PATCH {status}` invece di cancellazione — non esiste `DELETE /users`).
