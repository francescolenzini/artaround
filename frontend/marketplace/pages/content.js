...
export async function init() {
  ...
  const search = searchInput('Cerca per titolo, artista, stile…', (v) => {
    state.search = v;
    state.page = 1;
    reload();
  });
...
export function openArtworkForm(artwork, reload) {
  const isNew = !artwork;
  const form = buildForm([
    ...
    { name: 'category', label: 'Categoria', colSpan: 1, value: artwork?.category },
    { name: 'style', label: 'Stile', colSpan: 1, value: artwork?.style },
    ...
    { name: 'materials', label: 'Materiali', value: (artwork?.materials || []).join(', '), help: 'separati da virgola' },
    { name: 'tags', label: 'Tag', value: (artwork?.tags || []).join(', '), help: 'separati da virgola' },
...
export function openItemForm(artwork, item, reload) {
  ...
  const form = buildForm([
    ...
    { name: 'languageCode', label: 'Codice lingua', colSpan: 1, value: c.languageCode || 'it' },
    ...
    { name: 'isFree', label: 'Gratuito', type: 'checkbox', value: item ? item.isFree : true, colSpan: 1 },
    { name: 'priceValue', label: 'Prezzo', type: 'number', colSpan: 1, value: price.value, min: 0, step: 0.01, help: 'solo se a pagamento' },
    { name: 'priceCurrency', label: 'Valuta', colSpan: 1, value: price.currency || 'EUR' },
    ...
    { name: 'supportsScreen', label: 'Supporta schermo', type: 'checkbox', value: rnd.supportsScreen !== false, colSpan: 1 },
    { name: 'supportsTTS', label: 'Supporta TTS', type: 'checkbox', value: rnd.supportsTTS !== false, colSpan: 1 },
  ]);
