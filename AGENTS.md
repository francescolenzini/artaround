...
Questo repository è un **umbrella Git**: orchestra tre componenti, ciascuno un repo autonomo collegato come submodule sotto `services/`, più la build di produzione assemblata. Ogni submodule ha il codice runtime in `app/` e la propria dockerizzazione in `docker/`.
...
docs/  (nell'umbrella)
  ...
  claude-project-instructions.md   istruzioni per il Project companion (claude.ai)
