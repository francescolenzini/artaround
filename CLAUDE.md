...
- **Navigator — navigazione**: niente tab bar. Tre regole non negoziabili, implementate in `components/Nav.tsx` (dettaglio in `docs/knowledge-base.md` §5f): indietro **solo** in alto a sinistra con etichetta che nomina la destinazione (`BackLink`); mappa come overlay globale (`MapPill`, chiusura con `history.back()` ed etichetta dal search param `from`); account come popover sulle iniziali nell'header (`AccountMenu`), mai una route. Se aggiungi una schermata, riusa questi tre componenti invece di inventare un altro modo di tornare indietro. Corollario: **le icone non sostituiscono le etichette di navigazione**. Un pittogramma può affiancare il testo (chip logistici del player), mai prenderne il posto lasciando la destinazione nel solo `aria-label`.
- **Navigator — mappa**: il corpo è `components/MapView.tsx`, condiviso dalle due route `/map/$visitId` e `/map` (senza visita). Zoom e pan stanno in `lib/useMapZoomPan.ts` (Pointer Events, nessuna dipendenza): non reimplementarli nella route. Due invarianti da non rompere — immagine e pin vivono nello **stesso stage** trasformato, così i pin in `%` restano allineati senza ricalcoli; il counter-scale `1/scale` del pin va **prima** delle traslazioni, altrimenti centraggio e offset anti-sovrapposizione crescono con la pianta. Lo spread dei pin coincidenti è in **pixel** (`layoutPins`), non in percentuale della mappa: in percentuale dipende dalla larghezza dell'immagine. Dettaglio in `docs/knowledge-base.md` §5h.
